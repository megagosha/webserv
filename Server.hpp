...
public:
	...
	void process_requests(std::pair<int, struct kevent *> &updates)
	{
		...
		while (i < updates.first)
		{
			...
			if (it == servers.end() && updates.second[i]->filter == EVFILT_READ)
			{
				ity = connections.find(updates.second[i]->ident);
				if (ity == connections.end())
				{
			...
			if (updates.second[i]->flags & EV_EOF && updates.second[i]->filter == EVFILT_READ)
			{
				close(updates.second[i]->ident);
				kq.deleteFd(updates.second->ident, true);
				connections.delete(updates.second[i]->ident);
			}
			i++;
		}
	}
		...
		while (i < updates.first)
		{
			if (updates.second[i]->filter == EVFILT_WRITE)
			{
				...
				if (it != pending_response.end())
				{
					...
					kq.deleteFd(updates.second->ident, true);
					connections.erase(updates.second[i]->ident);
				}
			}
			++i;
		}
