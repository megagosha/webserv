...
//    std::string path_info = absolute_path.substr(absolute_path.find_last_of('/') + 1);
	...
	_cgi_env["REQUEST_URI"]     = request.getRequestUri();
	...
	if (it == request.getHeaderFields().end())
		_cgi_env["SERVER_NAME"] = "0"; //@todo check logic Maybe get server name from VirtualServer config
	else
...
bool HttpResponse::ParseCgiHeaders(size_t end)
{
	...
	while (pos < end && _body.find("\r\n\r\n", pos) != pos)
	{
		...
		if (!Utils::parse(_body, ++pos, "\r\n", false, MAX_VALUE, field_value))
			break;
		_response_headers.insert(std::map<std::string, std::string>::value_type(field_name, field_value));
	}
	if (pos != end)
	{
...
//		kq.addProcess(nChild);
		...
		while ((res = kq.getUpdates(5)).first >= 0)
		{
...
//		} else
//		{
//			size_t dif = v_it - _body.begin() + 4;
//			insertHeader("Content-Length", std::to_string(_body.size() - dif));
//		}
...
//		write(STDOUT_FILENO, _body.data(), _body.size());
	...
	if (req->getMethod() == "HEAD")
		return (EXIT_SUCCESS);
//	if (_body_size > 0)
//		send(fd, _body.data(), _body_size, 0);
